...
export default async function handler(req, res) {
  ...
  try {
    ...
    const file = files.comprovante?.[0];
    ...
    const uploadsDir = path.join('/tmp', 'uploads', 'comprovantes');
    ...
    const filename = `comprovante-${uniqueSuffix}${ext}`;
    ...
    if (bill.comprovante_file) {
      const oldPath = path.join(uploadsDir, bill.comprovante_file);
    ...
    const updateData = {
      comprovante_file: filename,
      comprovante_filename: file.originalFilename || filename,
      updatedAt: new Date()
    };
    ...
    res.status(200).json({ message: 'Comprovante enviado com sucesso', bill: formattedBill });
  } catch (error) {
